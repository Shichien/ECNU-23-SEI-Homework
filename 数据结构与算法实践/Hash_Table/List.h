...
public:
    ...
    Error_List_Code replace(int position, const List_entry &x); // replaces the entry at position in the List with x

    Error_List_Code remove(int position, List_entry &x); // removes the entry at position in the List and copies it to x

    Error_List_Code insert(int position, const List_entry &x); // inserts x into the List at position
